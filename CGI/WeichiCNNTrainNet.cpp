...
void WeichiCNNTrainNet::init( bool bDisplayErrorMessage/*=false*/ )
{
	...
	m_sl_label_size = m_caffe_net->blob_by_name("label_sl")->count(1);
...
void WeichiCNNTrainNet::update( bool bEvaluateFirstBatch )
{
	...
	if (iter % WeichiConfigure::snapshot == 0) {
		saveNetWork(WeichiConfigure::snapshot_prefix + std::to_string(iter) + ".pb");
	}
#endif
	if (iter % WeichiConfigure::display == 0) {
		CERR() << getTrainingInfo();
	}
}
