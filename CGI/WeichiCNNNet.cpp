...
{
	double dMinKomi = 7.5 - (getVNLabelSize() - 1) / 2;
	double dMaxKomi = 7.5 + (getVNLabelSize() - 1) / 2;
...
{
	int komi_diff = static_cast<int>(dKomi - 7.5);
...
int WeichiCNNNet::getMLVNKomiIndex(double dKomi, int mlvn_label_size)
{
	int komi_diff = static_cast<int>(dKomi - WeichiConfigure::komi);
	int komi_index = (mlvn_label_size - 1) / 2 + komi_diff;
	if (komi_index < 0) { komi_index = 0; }
	else if (komi_index >= mlvn_label_size) { komi_index = mlvn_label_size - 1; }
