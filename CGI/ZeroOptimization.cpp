...
void ZeroOptimizationSlave::calculateFeatures()
{
	...
	if (cnnTrainNet->hasSLOutput()) {
		...
		*fLabel = static_cast<float>(WeichiMove::toCompactPosition(move.getPosition()));
		...
		if (cnnTrainNet->hasSLDistribution()) {
			string sMCTSDistribution = (rand_moves < sgfLoader.getComment().size()) ? sgfLoader.getComment()[rand_moves] : "";
	...
	if (cnnTrainNet->hasVNOutput()) {
		string sResultInfo = sgfLoader.getResultInfo();
...
bool ZeroOptimizationMaster::initialize()
{
	...
	m_sharedData.m_trainNet = new WeichiCNNTrainNet(param, "");
...
void ZeroOptimizationMaster::runOneOptimization()
{
	...
	/*int nMaxIteration = m_sharedData.m_selfplayGameRecords.size() * (WeichiConfigure::TotalGrids * 2 / 3) / (WeichiConfigure::zero_replay_buffer_size * train_net_batch_size);
	nMaxIteration /= WeichiConfigure::zero_optimization_num_worker;*/
	int nMaxIteration = 1200;	// hard-code for 19x19 training
	for (int iteration = 0; iteration < nMaxIteration; iteration++) {
		...
		if (m_sharedData.m_trainNet->getIteration() % WeichiConfigure::zero_optimization_snapshot_iter == 0) {
			...
			/*if (!WeichiConfigure::zero_optimization_quiet) {
				string sFileName = "iter_" + ToString(m_sharedData.m_trainNet->getIteration()) + ".pb";
...
{
	if (WeichiConfigure::zero_optimization_sgf_directory == "") { return; }

	for (; m_optimizaionIteration <= WeichiConfigure::zero_optimization_sgf_max_iteration; m_optimizaionIteration++) {
		string sFileName = WeichiConfigure::zero_optimization_sgf_directory + "/" + ToString(m_optimizaionIteration) + ".sgf";
		loadSgf(sFileName);
	}
}
