...
vector<string> WeichiGlobalInfo::getSearchTreeSummaryString( const WeichiBoard& board )
{
	...
	ss = "  SL Queue Size: " + ToString(m_CNNJobQueue.getQueueSize()) + "; SL Job done: " + ToString(m_treeInfo.m_nDCNNJob) + "(" + ToString(m_treeInfo.m_nDCNNJob/m_treeInfo.m_dThinkTime) + " node/s)";
	vString.push_back(ss);
